...
//
//
// Related Topics 树 深度优先搜索 设计 哈希表 👍 80 👎 0


package com.jue.java.learn.leetcode.editor.cn.ThroneInheritance;
...
class ThroneInheritanceTest {
    ...
    public static void main(String[] args) {
        ThroneInheritance bean = new ThroneInheritance("king");
        bean.birth("king", "andy");
        bean.birth("king", "bob");
        bean.birth("king", "catherine");
        bean.birth("andy", "matthew");
        bean.birth("bob", "alex");
        bean.birth("bob", "asha");
        System.out.println(bean.getInheritanceOrder());
        bean.death("bob");
        System.out.println(bean.getInheritanceOrder());
    }
}
...
class ThroneInheritance {
    ...
    String kingName;
    Map<String, KingdomTree> store = new HashMap<>();

    public ThroneInheritance(String kingName) {
        this.kingName = kingName;
        KingdomTree root = new KingdomTree(kingName);
        store.put(kingName, root);
    }

    public void birth(String parentName, String childName) {
        KingdomTree parent = store.get(parentName);
        KingdomTree child = new KingdomTree(childName);
        parent.children.add(child);
        store.put(childName, child);
    }

    public void death(String name) {
        store.get(name).death = true;
    }

    public List<String> getInheritanceOrder() {
        // 深度遍历获取继承树；前序遍历
        return store.get(kingName).dfs();
    }

    static class KingdomTree {
        public String name;
        public boolean death = false;
        public List<KingdomTree> children = new ArrayList<>();

        public KingdomTree(String name) {
            this.name = name;
        }
        ...
        public List<String> dfs() {
            ...
            for (KingdomTree child : children) {
                result.addAll(child.dfs());
            }
            return result;
        }
    }
