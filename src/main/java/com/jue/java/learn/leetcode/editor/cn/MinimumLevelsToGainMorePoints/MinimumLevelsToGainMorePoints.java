...
// 
//
// Related Topics 数组 前缀和 👍 19 👎 0


package com.jue.java.learn.leetcode.editor.cn.MinimumLevelsToGainMorePoints;
...
public class MinimumLevelsToGainMorePoints {
    public static void main(String[] args) {
        ...
        System.out.println(solution.minimumLevels(new int[]{1, 1, 1, 1, 1}));// 3
        System.out.println(solution.minimumLevels(new int[]{0, 0, 0}));// 1
        System.out.println(solution.minimumLevels(new int[]{1, 0, 0}));// 1
    }
}
...
class Solution {
    public int minimumLevels(int[] possible) {
        ...
        int alice = count > 0 ? count / 2 : ((count - 1) / 2);
        for (Map.Entry<Integer, Integer> entry : firstIndex.entrySet()) {
            if (entry.getKey() > alice) {
