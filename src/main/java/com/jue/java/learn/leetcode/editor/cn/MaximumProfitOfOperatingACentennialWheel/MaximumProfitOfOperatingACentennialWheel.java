...
package com.jue.java.learn.leetcode.editor.cn.MaximumProfitOfOperatingACentennialWheel;
...
public class MaximumProfitOfOperatingACentennialWheel {
    public static void main(String[] args) {
        ...
        System.out.println(solution.minOperationsMaxProfit(new int[]{8, 3}, 5, 6)); // 3
    }
}
...
class Solution {
    public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
        ...
        while (remain > 0 || index < len) {
            if (index < len) {
                remain += customers[index];
            }
            current -= runningCost;
            int spend = Math.min(4, remain);
            remain -= spend;
            current += boardingCost * spend;
            if (current > max) {
                max = current;
                result = index + 1;
            }
