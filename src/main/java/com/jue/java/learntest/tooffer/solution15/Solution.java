package com.jue.java.learntest.tooffer.solution15;
...
public class Solution {
    ...
    public ArrayList<String> Permutation(String str) {
        ...
        for (int i = 0; i < (len = str.length()); i++) {
            int index = 0;
            for (int j = 0; j < len; j++) {
                if ((int) str.charAt(j) < (int) str.charAt(index)) {
                    index = j;
                }
            }
            sort += str.charAt(index);
            str = str.substring(0, index) + str.substring(index + 1, len);
        }
        return proess(sort + str);
    }

    public ArrayList<String> proess(String str) {
        ArrayList<String> result = new ArrayList<>();
        int len;
        if ((len = str.length()) <= 1) {
            result.add(str);
        ...
        char last = ' ';
        for (int i = 0; i < len; i++) {
            char _this = str.charAt(i);
            if (_this != last) {
                ArrayList<String> temp = proess(str.substring(0, i) + str.substring(i + 1, len));
                for (String t : temp) {
                    result.add(str.charAt(i) + t);
                }
            }
            last = _this;
        }
